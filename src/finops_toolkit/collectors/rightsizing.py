...
def _advisory(region: str, account_id: str) -> Detection:
    return Detection(
        ...
        region=region,
        evidence="AWS Compute Optimizer returned no data (not opted in or still warming up).",
...
def collect(session: boto3.Session, region: str, account_id: str) -> list[Detection]:
    ...
    try:
        ...
        return [_advisory(region, account_id)]
    ...
    return out or [_advisory(region, account_id)]
